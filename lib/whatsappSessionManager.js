...
async function saveMessageToSupabase(messageData) {
    ...
    const {
        userId,
        messageChatJid, // This is the internal variable for the JID of the chat
        chatName,
        content,
        messageType,
        timestamp,
        ...
        receiverName,
        deliveryStatus,
        fullMessageObject, // This is the raw message.message object
    ...
    while (!saved && retries < maxRetries) {
        try {
            ...
            saved = true;
            logger.info('✅ Saved message to Supabase');
        } catch (error) {
            retries++;
            logger.error({ error, userId, messageChatJid }, `❌ Failed to save message (attempt ${retries}):`);
    ...
    if (!saved) {
        logger.error({ userId, messageChatJid }, `Fatal: Failed to save message after ${maxRetries} attempts.`);
